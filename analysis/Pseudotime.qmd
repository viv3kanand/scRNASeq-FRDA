...
df_cad <- bind_cols(
  ...
  as.data.frame(cad_ncad@meta.data[,c(6,26, 29)])
  ) |> 
  sample_frac(1)
...
ggplot(df, aes(x = umapharmony_1, y = umapharmony_2, col = scores)) +
    geom_point(size = .7) +
    scale_color_viridis_c(option = "C") +
    labs(col = "Scores")

ggplot(df_cad, aes(x = umapharmony_1, y = umapharmony_2, col = scores)) +
    geom_point(size = .7) +
    scale_color_viridis_c(option = "C") +
    labs(col = "Scores")
```
