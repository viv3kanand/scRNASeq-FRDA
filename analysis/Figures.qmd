...
seu_PBMC <- Seurat::ScaleData(seu_PBMC, scale.max = 1000000)
...
```{r}
# Il-10 levels in FA vs HC

seu_PBMC@meta.data |> 
    select(orig.ident, Condition, cell_type, 30) |>
    dplyr::filter(cell_type == "CD14 Mono") |> 
    pivot_longer(cols = c(-orig.ident, -Condition, -cell_type ), names_to = "source", values_to = "score") |> 
    ggplot(aes(Condition, score)) +
    geom_boxplot(aes(fill = cell_type), width = 0.5, outlier.colour = "grey") + 
