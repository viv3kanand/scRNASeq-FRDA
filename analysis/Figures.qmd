...
P1B <- ipa_bulk |> 
  ...
  scale_fill_manual(values = c("#CB477A", "#469FBF")) +
...
res_gsva_test <- res_gsva |> 
  pivot_longer(cols = 1:8, names_to = "celltype", values_to = "value") |> 
...
P1C <- res_gsva |> 
  pivot_longer(cols = 1:8, names_to = "celltype", values_to = "value") |>
  ggplot() +
  geom_boxplot(aes(Condition, value, fill = Condition), alpha = 0.8, width = 0.5, outlier.colour = "grey", fatten = 1) +
  facet_wrap(~celltype) +
  guides(alpha = "none") +
  ggpubr::stat_pvalue_manual(res_gsva_test, label = "p.adj", size = 2) +
  scale_fill_manual(values = c("#CB477A", "#469FBF")) +
  labs(x = "", y = "GSVA score") +
  custom_theme() +
...
P1E <- wgcna_bulk |> filter(MEs %in% wgcna_test$MEs) |>
  ...
  ggpubr::stat_pvalue_manual(wgcna_test, label = "p.adj", size = 2, y.position = 0.5) +
  facet_wrap(~MEs) + 
  scale_fill_manual(values = c("#CB477A", "#469FBF")) +
...
st <- lm(log2FoldChange ~ log2FC, data = dge_combined)

dge_combined |> 
    mutate(
        Expression = case_when(
            log2FoldChange > 0 & log2FC > 0 & padj < 0.05 & pval < 0.05 ~ "Upregulated",
            log2FoldChange < 0 & log2FC < 0 & padj < 0.05 & pval < 0.05 ~ "Downregulated",
            TRUE ~ "Not significant")) |> 
    ggplot(aes(log2FoldChange, log2FC)) +
    geom_point(aes(color = Expression, alpha = 0.8)) +
    scale_color_manual(values = c("#CB477A", "grey", "#469FBF")) +
    geom_hline(yintercept = c(-0.5, 0.5), linetype = 2, color = "black") +
    geom_vline(xintercept = c(-0.5, 0.5), linetype = 2, color = "black") +
    guides(alpha = "none", size = "none") + 
    custom_theme()

```

### Panel 2: scRNA-Seq summary - PBMC & WB
...
P2A <- umap_plot(data = seu_meta, group = cell_type)

# UMAP - CAD-NCAD

P2B <- umap_plot(data = seu_meta[!is.na(seu_meta$Condition2),], group = cell_type_major) +
...
P2B <- DotPlot(seu_PBMC, features = markers_PBMC, dot.scale = 3) + 
...
summary_df_pbmc <- lapply(names(DGE_pbmc_filt), function(cell_type) {
...
P2C <- summary_df_pbmc |> 
  ...
  scale_fill_manual(
    ...
    breaks = rev(setdiff(unique(summary_df_pbmc$Regulation), "Regulation"))
    ) +
  guides(
...
P2E <- DotPlot(seu_WB, features = markers_WB, dot.scale = 3) + 
  scale_colour_gradient2(low = "#3CB49F", mid = 'white', high = "#5575B0") +
  RotatedAxis() +
  custom_theme() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) &
...
DGE_wb <- read_rds("RData/2024-09-26_DGE_sc_WB.rds")

DGE_wb_filt <- lapply(DGE_wb, function(df){
  df |> filter(abs(avg_log2FC) > 1 & p_val_adj < 0.05)
})

summary_df_wb <- lapply(names(DGE_wb_filt), function(cell_type) {
  df <- DGE_wb_filt[[cell_type]]
  up <- nrow(filter(df, avg_log2FC > 1 & p_val_adj < 0.05))
  down <- nrow(filter(df, avg_log2FC < -1 & p_val_adj < 0.05))
  data.frame(
    CellType = cell_type,
...
P2F <- summary_df_wb |> 
  ...
  scale_fill_manual(
    ...
    breaks = rev(setdiff(unique(summary_df_wb$Regulation), "Regulation"))
    ) +
  guides(
    fill = guide_legend(nrow = 1)
    ) +
...
P2_left <- cowplot::plot_grid(P2A, P2D, ncol = 1, labels = c("A", "D"), axis = "b", align = "hv")

P2_mid <- cowplot::plot_grid(P2B, P2E, ncol = 1, labels = c("B", "E"), axis = "b", align = "hv")

P2_right <- cowplot::plot_grid(P2C, P2F, ncol = 1, labels = c("C", "F"), axis = "b", align = "hv")

P2_final <- cowplot::plot_grid(P2_left, P2_mid, P2_right, nrow = 1, rel_widths = c(0.8, 1, 0.8), axis = "lr", align = "hv")
...
### Panel 3: scRNA-Seq - PBMC - DCATS, Trajectory, CD14 Mono, GSEA, TF

### Edit - Trajectory, CD14 Volcano, GSEA, AUC Model, IL10 Proportion
...
# 
# P3A <- ggplot(dcats_df, aes(x = cell_type, y = n, fill = Condition)) +
#   geom_col(position = "fill", width = 0.9, alpha = 0.8) +
...
P3C <- colData(sce_cd14)[,c(1,2,3,7,8)] |>
  as.data.frame() |> 
...
P3BC <- cowplot::plot_grid(P3B, P3C, ncol = 1, labels = c("B", "C"), rel_widths = c(1, 1), axis = "bt", align = "hv")

P3_top <- cowplot::plot_grid(P3A, P3BC, nrow = 1, labels = c("A"), rel_widths = c(1, 1))

# P3_top <- cowplot::plot_grid(P3A, P3BCD, nrow = 1, labels = c("A"), rel_widths = c(0.5, 1))
...
P3E <- do.call("rbind", GSEA_CD14) |> 
  ...
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
  scale_fill_manual(values = c("Enriched" = "#CB477A", "Depleted" = "#469FBF"), breaks = c("Enriched", "Depleted")) +
  custom_theme()


P3_mid <- cowplot::plot_grid(P3D, P3E, nrow = 1, labels = c("D", "E"))
...
#   xlab("-log10(p-adjust)") +
#   ylab("Top 20 TF") +
#   scale_fill_gradientn(colours = c('#FEE08B', '#CB477A'), na.value = "grey") +
#   custom_theme()



# IL10 Module score (update with AUCell)
...
#   geom_boxplot(width = 0.1, outlier.colour = "grey") + 
...
P3F <- seu_meta |> 
    select(orig.ident, Condition, cell_type, IL10, IL10RA, IL10RB) |> 
    tidyr::pivot_longer(cols = c(-orig.ident, -Condition, -cell_type ),
                        names_to = "gene", 
    ...
    labs(x = "Condition", y = "Scaled Expression") + 
    guides(fill = "none") +
    scale_fill_manual(values = c("#469FBF", "#CB477A")) +
    custom_theme()




P3G <- cowplot::ggdraw() + cowplot::draw_image("figures/Panel3/AUC_image.png")


# IL10 expressing cell proportion (update with AUCell)
...
P3H <- seu_meta |> 
    filter(cell_type == "CD14 Mono") |>
    ggplot(aes(x = Condition, fill = IL10_active)) +
    geom_bar(position = "fill", width = 0.5, alpha = 0.8, colour="black") +
    scale_fill_manual(values = c("gray80", "#CB477A")) +
    ylab("Proportion of Cells") +
    ggtitle("IL-10 Active Cells per Condition") +
    custom_theme()


auc_thresholds <- read_rds("figures/Panel3/auc_thresholds.rds")
cells_AUC <- read_rds("figures/Panel3/cells_AUC.rds")

threshold <- auc_thresholds$REACTOME_INTERLEUKIN_10_SIGNALING$aucThr$selected

active_cells <- colnames(seu_PBMC)[
  getAUC(cells_AUC)["REACTOME_INTERLEUKIN_10_SIGNALING", ] > threshold
]

# Add activity classification to metadata
seu_PBMC$IL10_active <- colnames(seu_PBMC) %in% active_cells

DimPlot(seu_PBMC, group.by = "IL10_active", reduction = "umap.harmony") +
  ggtitle("Interleukin-10 Signaling Activity (AUCell Thresholded)")

## Create the Nebulosa density plot instead of Dimplot
P3I <- plot_density(seu_PBMC, "IL10_active", reduction = "umap.harmony", joint = FALSE) +
  scale_color_viridis_c(option = "plasma") +
  ggtitle("Interleukin-10 Signaling Activity") +
  custom_theme() +
  empty_axis()



P3_bottom <- cowplot::plot_grid(P3F, P3G, P3H, P3I, nrow = 2, labels = c("F", "G", "H", "I"), align = "hv")

# P3_bottom <- cowplot::plot_grid(P3GH, P3H, nrow = 1, labels = c("","H"),  rel_widths = c(0.4, 0.6))

P3_top_mid <- cowplot::plot_grid(P3_top, P3_mid, ncol = 1, align = "hv")

P3_final <- cowplot::plot_grid(P3_top_mid, P3_bottom, ncol = 1, align = "hv", rel_heights = c(1,1))
...
### Panel 4: Pathways, TF, IFN-RIG1, TF-STAT1

```{r}
# top pathway terms in clusters

wb_top_pval <- read.csv("RData/WB/WB_top_pval.csv")

# col_fun = colorRamp2(c(0, 2, 20),
#                      c("grey", "#FEE08B", "#CB477A"))
# 
# P4A <- ggplotify::as.ggplot(Heatmap(as.matrix(wb_top_pval), 
#                                     cluster_rows = TRUE, 
...
P4A <- wb_top_pval |> 
  pivot_longer(cols = -Description, 
               names_to = "cell_type", 
               values_to = "log10pval") |> 
  mutate(log10pval = na_if(log10pval, 0),
         cell_type = str_replace(cell_type, "\\.", " ")) |> 
  ggplot(aes(x = cell_type, y = Description, fill = log10pval)) +
  geom_tile() +
  labs(x = NULL, y = NULL) +
  scale_fill_gradient(low = "#FEE08B", high = "#CB477A", na.value = "grey") +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
...
seu_WB_neu <- subset(seu_WB, subset = Condition %in% c("Control", "Patient"), idents = "Neutrophil")

expr_matrix_neu <- GetAssayData(seu_WB_neu, slot = "data")

DefaultAssay(seu_WB_neu) <- "module_score"

cell_metadata <- t(as.matrix(seu_WB_neu@assays$module_score@data)) |> 
    as.data.frame() |> 
    dplyr::mutate(cluster = Seurat::Idents(seu_WB_neu),
                  Condition = seu_WB_neu@meta.data$Condition,
                  LY6E = expr_matrix_neu["LY6E",])
...
P4ABC <- cowplot::plot_grid(P4A, P4B, P4C, nrow = 1, labels = "AUTO", rel_widths = c(1,0.5,0.5), align = "h", axis = "tb")
...
acts_TF_WB <- read_rds("../data/temp/acts_TF_WB.rds")

seu_WB[['tfsulm']] <- acts_TF_WB |> 
...
P4_top <- cowplot::plot_grid(P4A, P4B, nrow = 1, rel_widths = c(0.6, 1), align = "hv", axis = "tb", labels = c("A", "B"))
...
cell_metadata <- t(as.matrix(seu_WB_neu@assays$module_score@data)) |> 
    as.data.frame() |> 
    dplyr::mutate(
                  cluster = Seurat::Idents(seu_WB_neu),
                  Condition = seu_WB_neu@meta.data$Condition,
                  LY6E = expr_matrix_neu["LY6E",],
                  STAT1 = expr_matrix_neu["STAT1",])
...
P4E <- cell_metadata |> 
    mutate(LY6E = if_else(LY6E > 0, TRUE, FALSE),
           STAT1 = if_else(STAT1 > 0, TRUE, FALSE)) |>
    select(LY6E, STAT1, Condition, cluster) |> 
    tidyr::pivot_longer(cols = c(-cluster, -Condition),
                        names_to = "source", 
                        values_to = "status") |> 
    ggplot(aes(x = Condition, y = as.numeric(status), fill = Condition)) +
...
P4_mid <- cowplot::plot_grid(P4C, P4D, P4E, nrow = 1, labels = c("C", "D", "E"))
...
P4F <- Nebulosa::plot_density(seu_WB_neu, 
             features = "IFN1", 
             joint = FALSE, 
             combine = TRUE,
             size = 0.5) + 
  labs(title = "IFN", x = "UMAP1", y = "UMAP2") +
  custom_theme() +
  empty_axis()

DefaultAssay(seu_WB_neu) <- "RNA"

P4GHI <- plot_density(seu_WB_neu, c("LY6E", "STAT1"), joint = TRUE, combine = TRUE, size = 0.5)

P4G <- P4FGH[[1]] + 
  labs(title = "LY6E", x = "UMAP1", y = "UMAP2") +
  custom_theme() +
  empty_axis()
  
P4H <- P4FGH[[2]] +
  labs(title = "STAT1", x = "UMAP1", y = "UMAP2") +
  custom_theme() +
  empty_axis()

P4I <- P4FGH[[3]] +
  labs(title = "LY6E + STAT1", x = "UMAP1", y = "UMAP2") +
  custom_theme() +
  empty_axis()

P4_bottom <- cowplot::plot_grid(P4F, P4G, P4H, P4I, nrow = 1, align = "v", axis = "tb", labels = c("F", "G", "H", "I"))


P4_final <- cowplot::plot_grid(P4_top, P4_mid, P4_bottom, ncol = 1, rel_heights = c(0.7,0.5, 0.5))
...
### Panel 5 - CAD vs NCAD : CS-SCORE, NLR
...
P5B <- seu_meta |>
  filter(cell_type == "CD14 Mono" & Condition2 %in% c("CAD", "NCAD")) |>
...
P5B <- seu_meta |> 
    filter(cell_type == "CD14 Mono" & Condition2 != "Control" & !is.na(Condition2)) |>
    ggplot(aes(x = Condition2, fill = IL10_active)) +
    geom_bar(position = "fill", width = 0.5, alpha = 0.8, colour="black") +
    scale_fill_manual(values = c("gray80", "#CB477A")) +
    labs(y = "IL10 Proportion", fill = "IL10 active") +
    custom_theme()


seu_meta |>
    filter(cell_type == "CD14 Mono" & Condition2 %in% c("CAD", "NCAD")) |>
    dplyr::select(orig.ident, Condition2, IL10_active) |>
    group_by(orig.ident, Condition2) |>
    summarise(prop = length(which(IL10_active == TRUE))/length(IL10_active)) |>
    ggplot(aes(Condition2, prop, fill = Condition2)) +
    geom_boxplot(width = 0.5, alpha = 0.8, outlier.colour = "grey") +
    labs(x = NULL, y = "IL10 Proportion") +
    custom_theme() +
...
# P5C <- wrap_plots(plots[results_combined |> filter(cox_pval < 0.05 & logrank_pval < 0.05) |> pull(gene)])

P5C <- plots$IL10RB

vsd <- read_rds("RData/Bulk/2025-01-28_vsd.rds")

P5D <- assay(vsd)[,ind_meta] |> 
    as.data.frame() |> 
    ...
    geom_boxplot(width = 0.5, alpha = 0.8, outlier.colour = "grey") + 
    labs(x = NULL, y = "IL10RB expression", title = "IL10RB") +
    guides(fill = NULL) +
    scale_fill_manual(values = c("#CB477A", "#469FBF")) +
    custom_theme() +
    geom_signif(
...
P5_top <- cowplot::plot_grid(P5A, P5B, P5C, P5D, nrow = 1, labels = c("A", "B", "C", "D"), axis = "bt", align = "h")



P5_bottom <- cowplot::plot_grid(P5C, P5D, nrow = 1, labels = c("C", "D"), rel_widths = c(0.7, 0.3), axis = "bt", align = "h")

P5_final <- cowplot::plot_grid(P5_top, P5_bottom, ncol = 1, axis = "bt", align = "hv")
...
P5C <- CS_CD14$CAD_NCAD$M9 |> 
  ...
  scale_color_gradientn(colours = c('#CB477A', '#FEE08B'), 
...
P5_bottom <- cowplot::plot_grid(P5C, P5D, nrow = 1, labels = c("C", "D"), axis = "lr", align = "hv")
