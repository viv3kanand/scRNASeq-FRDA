...
P1AB <- cowplot::plot_grid(P1A, P1B, rel_widths = c(0.8, 1), align = "h")
...
res_gsva <- applyGSVA(gset_df = gset_celltype, group_col = "cell_type", gene_col = "symbol", expr_mat_list = list(vst_gsva), kcdf = "Gaussian")

res_gsva[[1]]$Condition <- colData(vsd)$Condition[match(rownames(res_gsva[[1]]), rownames(colData(vsd)))]



P1C <- res_gsva[[1]] |> 
  pivot_longer(cols = 1:26, names_to = "celltype", values_to = "value") |>
...
P1top <- cowplot::plot_grid(P1A, P1B, P1C, rel_widths = c(0.8, 1, 1), nrow = 1, align = "h", labels = "auto")
...
dcats_df <- read_rds("RData/2025-02-13_dcats-df.rds")


anno_df <- dcats_df[dcats_df$LRT_pvals < 0.05,] |> 
...
seu_meta |> 
    ...
    mutate(dcats = rep(dcats_df$LRT_pvals, each = 2),
           stars = case_when(
               ...
               TRUE ~ "")) |> 
    group_by(cell_type) |> 
    mutate(prop = n / sum(n)) -> dcats_plot

dcats_plot$cell_type <- reorder(dcats_plot$cell_type, ifelse(!dcats_plot$Condition %in% "FRDA", 0, dcats_plot$prop), FUN = sum)

ggplot(dcats_plot, aes(x = cell_type, y = n, fill = Condition)) +
