...
P1E <- wgcna_bulk |> filter(MEs %in% wgcna_test$MEs) |>
  ggpubr::ggboxplot(x = "condition", y = "value", fill = "condition", facet.by = "MEs", width = 0.5, size = 0.3) + 
...
P3G <- t(as.matrix(seu_WB@assays$tfsulm@data)) |> 
  as.data.frame() |> 
  dplyr::mutate(cluster = Seurat::Idents(seu_WB),
                Condition = seu_WB@meta.data$Condition) |> 
  tidyr::pivot_longer(cols = c(-cluster, -Condition), 
...
P2_bottom <- cowplot::plot_grid(NULL, P2F, P3G, nrow = 1, labels = c("E", "F", "G"), rel_widths = c(1, 1), axis = "b", align = "hv")
...
P3E <- ggplot(imb_df, aes(x = umapharmony_1, y = umapharmony_2, col = scores)) +
  ...
  scale_color_gradientn(colours = c("#5E4FA2", "#5575B0", "#469FBF", "#3CB49F", "#ABDDA4", "#FFFF33")) +
  labs(title = NULL, x = "UMAP1", y = "UMAP2") +
  custom_theme() +
  empty_axis()
