...
t(as.matrix(seu_WB@assays$module_score@data)) |> 
    ...
    ggplot(aes(Condition, score)) +
    ggdist::stat_halfeye(aes(fill = Condition, alpha = 0.8), adjust = .5, width = .7, justification = -.2, .width = 0, point_colour = NA) +
    geom_boxplot(aes(fill = Condition, alpha = 0.8), width = 0.1, outlier.colour = "grey") + 
    facet_grid(~source) +
    custom_theme() +
    scale_fill_manual(values = c("#3288BD", "#D53E4F")) +
    geom_signif(
