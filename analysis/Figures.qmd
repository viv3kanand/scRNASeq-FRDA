...
P1D <- wgcna_bulk |> filter(MEs %in% wgcna_test$MEs) |>
  ggpubr::ggboxplot(x = "condition", y = "value", fill = "condition", facet.by = "MEs") + 
  ggpubr::stat_pvalue_manual(wgcna_test) +
  scale_fill_manual(values = alpha(c("#469FBF", "#CB477A"), 0.8)) +
  labs(x = "", y = "Module Eigengene", fill = "Condition") +
  custom_theme() +
  theme(strip.placement = "outside", axis.text.x = element_blank())

P1D <- shift_legend(P1D)
