...
ego_bulk@result |> 
    ...
    scale_color_gradientn(colours = c('#FFFFFF', '#D53E4F'), limit = c(0,8), na.value = "grey") +
...
P2 <- ipa_bulk |> 
    head(20) |> 
    mutate(order = Ratio, .by = Pathways,
           Count = str_count(Molecules, ",") + 1) |> 
    ggplot(aes(x = Ratio, y = reorder(Pathways, order), color = Logpval, size = Count)) +
    geom_point() + 
    labs(x = "Gene Ratio", y = "Pathways") + 
    scale_color_gradientn(colours = c('#FFFFFF', '#D53E4F'), limit = c(0,27), na.value = "grey") +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
    custom_theme()

P2 <- ipa_bulk |> 
    head(20) |> 
    mutate(order = Logpval, .by = Pathways,
           Count = str_count(Molecules, ",") + 1) |> 
    ggplot(aes(x = Logpval, y = reorder(Pathways, order), color = Count, size = Count)) +
    geom_point() + 
    labs(x = "-log10(pvalue)", y = "Pathways") + 
    scale_color_gradientn(colours = c('#FFFFFF', '#D53E4F'), limit = c(0,64), na.value = "grey") +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
    guides(size = "none") +
    custom_theme()
...
P1AB <- cowplot::plot_grid(P1A, P1B, rel_widths = c(0.8, 1), align = "h", labels = "AUTO")

ggsave(filename = "figures/Bulk/fig1AB.png", 
       plot = P1AB,
       units = "in", 
       width = 8.5,
       height = 13/4,
...
ann_colors = list(Condition = c(Control = "#3288BD", Patient = "#D53E4F"))
...
h1 <- ggplotify::as.ggplot(
    pheatmap::pheatmap(mat = mat_ipa1,
                       annotation_col = meta_bulk |> select(Condition),
                       annotation_colors = ann_colors,
                       color = colors.use,
                       border_color = "white",
...
h1 <- ComplexHeatmap::Heatmap(t(scale(t(as.matrix(mat_ipa1)))), 
                        top_annotation = ha, 
...
h2 <- ggplotify::as.ggplot(
    pheatmap::pheatmap(mat = mat_ipa2,
                       annotation_col = meta_bulk |> select(Condition),
                       annotation_colors = ann_colors,
                       color = colors.use,
                       border_color = "white",
...
h2 <- ComplexHeatmap::Heatmap(t(scale(t(as.matrix(mat_ipa2)))), 
                        top_annotation = ha,
...
h3 <- ggplotify::as.ggplot(
    pheatmap::pheatmap(mat = mat_ipa3,
                       annotation_col = meta_bulk |> select(Condition),
                       annotation_colors = ann_colors,
                       color = colors.use,
                       border_color = "white",
...
h3 <- ComplexHeatmap::Heatmap(t(scale(t(as.matrix(mat_ipa3)))), 
                        top_annotation = ha, 
...
layout <- c(
    ...
    area(t = 1, l = 2, b = 1, r = 4),
    area(t = 1, l = 3, b = 1, r = 4)
)

h_list <- (ggplotify::as.ggplot(h1) | ggplotify::as.ggplot(h2) | ggplotify::as.ggplot(legend_heatmap)) + 
...
# P2A <- cowplot::ggdraw() + cowplot::draw_image("figures/Bulk/WGCNA/manu-fig-02-B-TOM-Dendrogram-25x9.png")

P2A <- cowplot::ggdraw() + cowplot::draw_image("figures/Bulk/WGCNA/manu-fig-02-A-TOMplot-30x30.png")

P2B <- cowplot::ggdraw() + cowplot::draw_image("figures/Bulk/WGCNA/manu-fig-02-C-boxplot-sig-padj-ME.png")
...
P2C <- ipa_m3 |> 
    ...
    ggplot(aes(x = Zscore, y = reorder(Pathways, order), fill = Enrichment, alpha = Logpval)) +
    geom_bar(aes(x = Zscore), stat = "identity") + 
    labs(x = "z-score", y = "Pathways", alpha = "-Log10(pvalue)") + 
    scale_fill_manual(values = c("#D53E4F", "#3288BD")) +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
    custom_theme()

P2AB <- cowplot::plot_grid(P2A, P2B, nrow = 1, rel_widths = c(1, 0.8))

P2AC <- cowplot::plot_grid(P2A, P2C, ncol = 1, rel_heights = c(0.4, 1))

cowplot::plot_grid(P2AC, P2B, nrow = 1, rel_widths = c(0.8, 1))

P2ABC <- cowplot::plot_grid(P2AB, P2C, nrow = 1, rel_widths = c(1, 0.6))


P1_P2 <- cowplot::plot_grid(P1, P2, rel_widths = c(0.8, 1), align = "h", labels = "AUTO")

P4_P5_P6 <- cowplot::plot_grid(P4, P5, P6, nrow = 1, labels = c("D", "E", "F"), rel_widths = c(0.5, 0.5, 1))

cowplot::plot_grid(P1_P2, P3, P4_P5_P6, ncol = 1, labels = c("", "C", ""))



P4_P5 <- cowplot::plot_grid(P4, P5, nrow = 1, labels = c("D", "E"), rel_widths = c(1, 1))
P4_P5_P6 <- cowplot::plot_grid(P4_P5, P6, ncol = 1, labels = c("", "G"), rel_heights = c(0.7, 1))
P3_P4_P5_P6 <- cowplot::plot_grid(P3, P4_P5_P6, nrow = 1, labels = c("C", ""))
cowplot::plot_grid(P1_P2, P3_P4_P5_P6, ncol = 1)

ggsave("analysis/figures/Bulk/panel.png", width = 20, height = 15, dpi = 600, bg = "white")
...
do.call("rbind", GSEA_CD14) |> 
    ...
    labs(title = NULL, x = "NES", y = "Gene Ontology") +
    scale_x_continuous(position="bottom") +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 50)) +
