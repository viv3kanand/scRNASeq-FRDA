...
ego_bulk@result |> 
    ...
    scale_color_gradientn(colours = c('#FFFFFF', '#009DBDFF'), limit = c(0,8), na.value = "grey") +
...
genes.to.label = c("FXN", "ISG15", "LY6E", "IFI6", "ISG20", "MX1", "IFIT2", "IFIT1", "CXCL10", "CCL8")
genes.to.label <- head(DGE_PBMC[["CD14 Mono"]][order(DGE_PBMC[["CD14 Mono"]][["p_val_adj"]]),], 10) |> pull(gene)

p <- ggplot(avg.cd14.mono, aes(Control, FRDA)) + geom_point() + ggtitle("CD14 Mono")

LabelPoints(p, points = genes.to.label, repel = TRUE)
...
CD14_plot_data |> 
    ...
    geom_point(aes(color = Expression)) + 
    ggtitle("CD14 Mono") + 
    ggrepel::geom_text_repel(data = head(CD14_plot_data |> filter(abs(avg_log2FC) > 2 & p_val_adj < 0.01 & !is.na(description)) |> arrange(p_val_adj, desc(abs(avg_log2FC))), 20), aes(label = gene), max.overlaps = 20, force = 10) +
    scale_color_manual(values = c("#FF8E7BFF", "grey", "#009DBDFF")) +
    custom_theme()

# pathway analysis (GSEA KEGG, BP)
...
tf_trrust |> 
  ...
  head(20) |> 
  ggplot(aes(x = -log(qvalue), y = reorder(TF, order), fill = genes)) +
  geom_bar(stat = "identity") +
  xlab("-log10(p-adjust)") +
  ylab("Top 20 TF") +
  scale_fill_gradientn(colours = c('#FFFFFF', '#FF8E7BFF'), limits = c(0, 30), na.value = "grey") +
  custom_theme()
...
t(as.matrix(seu_WB@assays$tfsulm@data)) |> 
  ...
  tidyr::pivot_longer(cols = c(-cluster, -Condition), 
                      names_to = "source", 
                      values_to = "score") |> 
  filter(source == "STAT1" & cluster == c("Neutrophil", "B cells", "CD4 Naive", "CD8 Naive")) |> 
  ggplot(aes(Condition, score)) +
...
seu_WB <- AddModuleScore(seu_WB, features = list(rig1_glist), name = "RIG1")
...
p1 <- CS_CD14$CAD_NCAD$M3 |> 
  ...
  scale_color_gradientn(colours = c('#009DBDFF', '#FFFFFF'), limit = c(min(CS_CD14$CAD_NCAD$M3$p.adjust), max(CS_CD14$CAD_NCAD$M3$p.adjust)), na.value = "grey") +
...
p2 <- CS_CD14$CAD_NCAD$M9 |> 
  ...
  scale_color_gradientn(colours = c('#009DBDFF', '#FFFFFF'), limit = c(min(CS_CD14$CAD_NCAD$M9$p.adjust), max(CS_CD14$CAD_NCAD$M9$p.adjust)), na.value = "grey") +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
  custom_theme()
...
p3 <- CS_CD14$CAD_NCAD$M15 |> 
    ...
    scale_color_gradientn(colours = c('#009DBDFF', '#FFFFFF'), limit = c(min(CS_CD14$CAD_NCAD$M15$p.adjust), max(CS_CD14$CAD_NCAD$M15$p.adjust)), na.value = "grey") +
    scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
    custom_theme()
...
CS_CD14$CAD_C$M8 |> 
    as_tibble() |>  
    dplyr::select(c(1:10)) |> 
    arrange(-log(p.adjust)) |> 
    mutate(ID = paste0(ONTOLOGY, ":", stringr::str_to_sentence(Description)),
           ID = factor(ID, levels = ID)) |> 
    ggplot(aes(x = -log(p.adjust), y = ID, fill = zScore)) +
    geom_bar(stat = "identity") +
    xlab("-log10(p-adjust)") +
    ylab("Module 8 Enrichment") +
    scale_fill_gradientn(rescaler = ~ scales::rescale_mid(.x, mid = 0), 
                         colours = c('#FF8E7BFF', '#FFFFFF', '#009DBDFF')) +
    # scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
    custom_theme()
...
CS_CD14$CAD_C$M8 |> 
    as_tibble() |>  
    dplyr::select(c(1:10,13)) |> 
    mutate(GR = sapply(parse(text = GeneRatio), eval),
           ID = paste0(ONTOLOGY, ":", stringr::str_to_sentence(Description)),
           GR = factor(GR, levels = unique(GR))) |> 
    arrange(GR) |>
    ggplot(aes(x = GR, y = ID, color = -log(p.adjust), size = Count)) +
    geom_point() +
  scale_fill_gradientn(colours = c("#FF8E7BFF", "#009DBDFF"), limits = c(-3, 3), na.value = "grey") +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
  custom_theme()
...
CS_CD14$CAD_C$M8 |> 
  ...
  mutate(GeneRatio = eval(parse(text = GeneRatio)),
         ID = paste0(ONTOLOGY, ":", stringr::str_to_sentence(Description)),
         order = GeneRatio, .by = ID) |> 
  ggplot(aes(x = GeneRatio, y = reorder(ID, order),color = -log10(p.adjust), size = Count)) +
  geom_point() + 
  labs(x = "Gene Ratio", y = "Gene Ontology") + 
  scale_color_gradientn(colours = c('#FFFFFF', '#009DBDFF'), limit = c(0,5), na.value = "grey") +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
  custom_theme()
